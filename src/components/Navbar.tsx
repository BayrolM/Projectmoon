...
export function Navbar() {
  ...
  const [isCruisesDropdownOpen, setIsCruisesDropdownOpen] = useState(false);
  ...
  const whatsappMessage =
    "¡Hola! Quisiera obtener información sobre los paquetes turísticos de Moon Travel Co. 🌙";
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-24">
          ...
          <div className="hidden md:flex items-center gap-8">
            ...
            <div
              className="relative"
              onMouseEnter={() => setIsCruisesDropdownOpen(true)}
              onMouseLeave={() => setIsCruisesDropdownOpen(false)}
            >
              <button
                style={linkStyle}
                className="flex items-center gap-1 text-gray-600 hover:text-[#512DDB] transition-colors duration-300"
              >
                Cruceros
                <ChevronDown
                  className={`w-4 h-4 transition-transform duration-300 ${isCruisesDropdownOpen ? "rotate-180" : ""}`}
                />
              </button>

              {isCruisesDropdownOpen && (
                <div className="absolute top-full left-0 pt-2 w-52 animate-fadeIn z-50">
                  <div className="bg-white rounded-xl shadow-xl border border-[#D2C3F7]/40 py-2">
